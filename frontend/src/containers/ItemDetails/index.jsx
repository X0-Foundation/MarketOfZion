...
const ItemDetailsContainer = (props) => {      
  ...
  function clickFavorite() {
    if (account) {
      if (!isLiking){
        ...
        setDidLike(i => !i)
        axios.post("/api/item/like", Querystring.stringify({address: account.toLowerCase(), tokenId: item.tokenId, collection: item.itemCollection}))
        .then(res => {
          setIsLiking(false)
        })
        .catch(err => {
  ...
  return (
    <>
      <section className="section-padding-100 detail-container">
                      ...
                      <div className="donnot-miss-widget">
                        <div className="who-we-contant">
                          <div className="filers-list">
                            <div to="/listed" className="filter-item mr-15" onClick={() => props.history.push(`/explore/?category=${item?.category}`)}>
                              {item?.category}
                            </div>
